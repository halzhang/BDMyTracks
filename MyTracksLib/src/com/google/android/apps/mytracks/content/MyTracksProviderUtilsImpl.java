...
public class MyTracksProviderUtilsImpl implements MyTracksProviderUtils {
   ...
   * @modify zhanghanguo add setGpsStatus
   */
  private void fillLocation(Cursor cursor, CachedTrackPointsIndexes indexes, Location location) {
    ...
    if (location instanceof MyTracksLocation && !cursor.isNull(indexes.gpsStatusIndex)) {
      MyTracksLocation myTracksLocation = (MyTracksLocation) location;
      myTracksLocation.setGpsStatus(cursor.getInt(indexes.gpsStatusIndex));
    }
  }
   ...
   * @modify zhanghanguo add {@link TrackPointsColumns.GPSSTATUS}
   */
  private ContentValues createContentValues(Location location, long trackId) {
    ...
    if (location instanceof MyTracksLocation) {
      ...
      Log.i(TAG, "GPS Status: "+myTracksLocation.getGpsStatus());
      values.put(TrackPointsColumns.GPSSTATUS, myTracksLocation.getGpsStatus());
    }
    return values;
  }
  ...
  private static class CachedTrackPointsIndexes {
    ...
    public CachedTrackPointsIndexes(Cursor cursor) {
      ...
      gpsStatusIndex = cursor.getColumnIndex(TrackPointsColumns.GPSSTATUS);
